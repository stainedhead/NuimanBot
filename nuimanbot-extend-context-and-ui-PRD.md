...
- **Reliability:** Run state (queue position, in-flight runs, Chore schedules, next-fire times) is persisted, not held only in memory — a server restart must not lose queued Jobs, drop an in-flight run's record, or cause a Chore to miss its next scheduled fire time.
...
| Project/Chat domain entities | Risk | No existing "Project" or "Chat session" concept in the domain layer — new entities, repositories, and persistence needed from scratch. |
