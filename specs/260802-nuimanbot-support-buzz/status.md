...
| Phase 3 | Tool integration (FR-010, FR-011, FR-012) | In Progress — P3.1/P3.2 complete, P3.3 remaining | 67% |
...
- [ ] P3.3 — Buzz-specific confirmation — not started.
...
**P3.1/P3.2 verification:** `internal/usecase/chat/rbac_test.go`'s `TestProcessMessage_RBACEnforcedAcrossPlatforms` (Telegram/Slack/CLI × RoleGuest-denied/RoleUser-allowed, 6 subtests, wired against the real `tool.Service` + `ratelimit.RateLimiter`, not mocks — now also asserting a denied tool is absent from the LLM's tool list, proving `ListTools` filtering) and `TestProcessMessage_RateLimitEnforced` both pass. `internal/usecase/tool/service_test.go`'s `TestListTools_RoleFiltering`/`TestListTools_AllowedToolsWhitelist` cover FR-012 directly. Buzz's case is still open, tracked under P3.3.
