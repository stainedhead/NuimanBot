...
// TestNewFileCredentialVault_FirstRunGeneratedKey reproduces the genuine
// first-run scenario: a key generated by crypto.GenerateEncryptionKey and
// base64-encoded for .env storage (crypto.EncodeKeyToBase64) must be
// base64-decoded (crypto.DecodeKeyFromBase64) back into a raw 32-byte key
// before vault construction, or NewFileCredentialVault rejects it.
func TestNewFileCredentialVault_FirstRunGeneratedKey(t *testing.T) {
	tempDir, err := os.MkdirTemp("", "nuimanbot-vault-firstrun-")
	if err != nil {
	...
	// This is exactly what is saved to .env and later read back from the
	// NUIMANBOT_ENCRYPTION_KEY environment variable.
	envValue := crypto.EncodeKeyToBase64(key)

	decodedKey, err := crypto.DecodeKeyFromBase64(envValue)
	if err != nil {
		t.Fatalf("DecodeKeyFromBase64 failed: %v", err)
	}

	filePath := filepath.Join(tempDir, "credentials.enc")
	vault, err := crypto.NewFileCredentialVault(filePath, decodedKey)
	if err != nil {
		t.Fatalf("NewFileCredentialVault failed with decoded first-run key: %v", err)
	}
	if vault == nil {
