...
**Known gap, not yet closed:** `Dequeue` removes and persists the removal of a `RunRequest` *before* `StubExecutor.Execute` runs it. A crash in the window between that dequeue and the run reaching a terminal status leaves the `Run` record permanently stuck at `Queued`/`Running` — the request is gone from `queue.json` (correctly, from the queue's own point of view) but nothing on startup scans `RunRepository` for orphaned non-terminal runs and requeues or fails them. This is a real, currently-unaddressed limitation of the Reliability NFR's "a restart must not... drop an in-flight run's record" goal, not a solved problem — the queue's *own* state is restart-durable; a run's fate *during* the dequeue-to-completion window is not yet.
...
This subsystem's pipeline (queueing, concurrency, cron evaluation, queued-work persistence, per-user WebSocket transport, path confinement, and IDOR-safe repository scoping) is implemented and tested end-to-end. What is **not** yet real: agent-invoking execution (`StubExecutor` is a placeholder), agent replies in the web Chats UI, the per-Job/Chore/Run/Memories chat interfaces, an automated retention sweep, a browser-side WebSocket consumer, and — per the Queue section above — recovery for a run that was already dequeued to a worker at the moment of a crash. See `documentation/product-summary.md`'s "Persistent Agent Workspace — In Progress" for the full, itemized list.
