...
| `Job` | `job.go` | One-time task: `Title`, `Description`, `HiddenDirectory` (holds `JOB-DESCRIPTION.md`), `ContextType`/`ContextID` (Chat or Project), `WorkingDirectory`, `Status` (`Queued`/`Running`/`Completed`/`Failed`), `PendingDeletion` (soft-delete marker, cleaned up by `jobs.Service.CleanupPendingDeletion` once the Job's Run reaches a terminal state). A prior `IsQueueable()` guard method was removed as dead code (FR-R14): the only enqueue path (`CreateJob`) always builds a brand-new, queueable-by-construction Job, so there was no caller for it to guard — reintroduce an equivalent check if a re-run/retry flow is ever added. |
...
### Web Chats Agent Wiring (2026-08-13)

Sending a message to an existing web Chat now invokes the real agent, closing the gap this section previously documented as a known limitation. This did not require a new subsystem — it required decoupling two things `chat.Service.ProcessMessage` had always bundled together: *which conversation thread* a message belongs to, and *who* (for RBAC purposes) is sending it.

**Why the existing entry point didn't fit.** Every other platform (Telegram, Slack, CLI, Buzz) has a one-conversation-thread-per-(platform, platform-user) model: `ProcessMessage` derives `conversationID` from `getConversationID(platform, platformUID)` and resolves the RBAC `domain.User` from that same pair automatically. The web Chats environment breaks that assumption on purpose — one logged-in user can own many independently-named Chat threads, each with its own UUID (`chats.Service.CreateChat`), which a single derived `platform:platformUID` conversation ID can't express.

**The fix:** `chat.Service` gained `ProcessMessageInConversation(ctx, conversationID string, user *domain.User, incomingMsg *domain.IncomingMessage) (domain.OutgoingMessage, error)` — a second entry point taking both values explicitly instead of deriving them, refactored to share `ProcessMessage`'s entire tail (confirmation-reply detection, tool-calling loop, turn completion) via a new unexported `processTurn`. `ProcessMessage`'s own behavior is unchanged — it computes `conversationID`/`user` the same way as before, then calls the same shared core.

`internal/usecase/chats.Service` gained `SendMessage(ctx, ownerUserID, chatID, content, role string) (domain.OutgoingMessage, error)`, wired via two new small interfaces (`ChatProcessor`, `UserService`, declared at point of use, satisfied by `*chat.Service`/`*user.Service` with no adapter needed) passed into `NewService`. It:
1. Enforces ownership via the existing `GetChat` (unchanged 404-not-403 IDOR posture).
2. Resolves/creates the RBAC `domain.User` for `(domain.PlatformWeb, ownerUserID)`, keeping its `Role` synced to the caller's *current* authenticated role on every call (`UserService.UpdateUserRole` if it's drifted) — deliberately not `chat.Service`'s own `defaultRoleForPlatform` Guest-default, since a web Chats user has already proven their identity via login, unlike an unauthenticated Telegram/Slack sender.
3. Calls `ProcessMessageInConversation(ctx, chatID, user, incoming)` — `chatID` doubles as `conversationID`, so the reply lands in the same thread. Both the user's message and the assistant's reply are persisted by `chat.Service`'s own `saveTurnMessages` internally; `SendMessage` does not separately call `AppendUserMessage` (that would double-persist the user's message).

`internal/adapter/web/chats_handler.go`'s `handleChatMessage` calls `SendMessage` (passing the authenticated session's own `Role`) instead of `AppendUserMessage`, recording `metrics.WebChatTurnsTotal{status="success"|"error"}` and `metrics.WebChatTurnDuration` around the call — mirroring the Buzz gateway's existing dedicated per-integration Prometheus metrics pattern (`internal/infrastructure/metrics/prometheus.go`) rather than introducing a different observability convention; both are scraped from the existing `/metrics` endpoint (`internal/infrastructure/health/server.go`) with no new wiring required. Structured logging is inherited for free — `processTurn` already logs via `requestid.Logger` with `platform`/`user`/`conversation_id` fields regardless of entry point.

**Timeout fix (found via live testing, not a design decision made in isolation):** `internal/adapter/web/server.go`'s `http.Server.WriteTimeout` was 15s — far shorter than a real LLM completion can take (`ollama.Client` alone allows up to 120s per call), so the connection was silently killed by the Go HTTP server out from under a chat turn that was still running and would have succeeded. Raised to 180s; a multi-iteration tool-calling turn (up to 5 iterations, `chat.Service.runToolLoop`) could still exceed even that in a pathological case — a known, unresolved trade-off of driving a synchronous LLM turn inside one HTTP request/response cycle, not something this fix fully closes.

**Known gap:** a brand-new Chat's first message (typed into the create-chat form, persisted directly by `CreateChat` for its auto-naming behavior) is not run through `SendMessage` — only messages sent to an already-existing Chat get a reply. Closing this would mean either changing `CreateChat`'s embed-the-first-message contract (used elsewhere, including the CLI, and covered by existing tests) or adding a "reply to already-persisted history without appending a new user message" mode to `chat.Service`, neither undertaken here — deliberately scoped out rather than risking `CreateChat`'s existing multi-consumer contract for a UX nicety. The CLI's parallel `/chat` environment (`internal/adapter/gateway/cli`, `AppendUserMessage`-only) was left unchanged — out of scope for this fix, which targeted the web Chats environment specifically.

### Known Limitations (Cross-Reference)

This subsystem's pipeline (queueing, concurrency, cron evaluation, queued-work persistence, per-user WebSocket transport with a live browser-side consumer, path confinement with symlink-escape mitigation, IDOR-safe repository scoping, restart recovery, and an automated retention sweep) is implemented and tested end-to-end. Agent replies in the web Chats UI are now real (see above, 2026-08-13); what is still **not** yet real: agent-invoking Job/Chore execution (`StubExecutor` is a placeholder), and the per-Job/Chore/Run chat interfaces (Memories now has one — see the Memories section — as the FR-R4 reference implementation the other three are meant to follow). The Memories `ownerUserID`→`ConversationID` identity-bridge gap noted in earlier revisions of this document is resolved for the CLI gateway as of CLI Parity (see the section below) — the CLI now requires real login and uses the same `ownerUserID` (session username) convention the web UI uses. Telegram/Buzz still use their own separate platform-identity systems, unaffected by either feature. See `documentation/product-summary.md`'s "Persistent Agent Workspace — In Progress" for the full, itemized list.
