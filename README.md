...
- ✅ The web Chats environment now generates real agent replies (2026-08-13) — sending a message invokes the full LLM/tool/RBAC pipeline (`chat.Service.ProcessMessageInConversation`) within that Chat's own thread, using the sender's actual authenticated role rather than defaulting to Guest; observable via the `web_chat_turns_total`/`web_chat_turn_duration_seconds` Prometheus metrics. One known gap: a brand-new Chat's *first* message (typed into the create-chat form) is still persisted only, not replied to — only messages sent to an already-created Chat get a reply; the CLI's parallel `/chat` environment is also still persistence-only.
